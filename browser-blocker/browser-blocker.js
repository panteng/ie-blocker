...
window.onload = function () {
    ...
    bbContainer = document.getElementById('bb-container');
    ...
    for (var i = 0; i < document.scripts.length; i++) {
        if (scriptPath = document.scripts[i].src.match(/(.*)browser-blocker\.js/)[1]) {
            break;
        }
    }
    for (var i = 0; i < browserIcons.length; i++) {
        if (browserName = browserIcons[i].className.match(/bb-ua-(\w+)/)[1]) {
